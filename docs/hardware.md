...
6. **磁力計擺放要求** — QMC6310 的固定位置、水平度、與 servo 的距離
...
heading 是 `atan2(By, Bx)` 的二維計算，**沒有傾角補償**。台灣磁傾角約 35°（垂直分量
約 26 µT、水平約 37 µT），板子一傾斜，垂直分量就會漏進水平面：
...
1. **磁力計 hard-iron**：按鈕啟動後把整台機器水平慢慢轉一圈。完成後看**擬合殘差**
   ——這就是你這個安裝位置的客觀評分。<1° 代表乾淨；好幾度代表 soft-iron 干擾
   （servo 的鋼齒輪），該把板子移遠再重測。也順便檢查**磁場強度**是否接近 0.37 G。
2. **地標校正**：servo 設 90°，從觀景窗把 1 km 外的地標對到畫面正中央，貼上它的
   座標。這一步鎖定 `mount_offset`，之後換浪點都不用再對準任何人。
...
2. **Servo 干擾（最關鍵）**：切手動，把角度滑桿從 0 拉到 180 再拉回來。
   **heading 不應該跟著動。** 若移動超過 1~2°，就是板子吃到 servo 的磁場或電流，
   把板子拉遠或把供電線雙絞。
3. **水平度**：把整個腳架水平轉 90°，heading 應該也變化約 90°。差太多代表板子沒擺平。
