...
| `history` | 過去 5 分鐘軌跡（每 1 秒一點，最多 300 點，循環 buffer）|
...
| `tempC10` | TelemetryPayload | `telemetry.temp_c` | ÷ 10（`INT16_MIN` → `null`）|
...
> `bearing` 不是封包欄位，而是攝影站用「自身 GPS」與「`client.lat/lon`」即時計算；`server.*` 來自攝影站本機 GPS 與感測器；`history` 為 `client.lat/lon` 每 1 秒取樣的循環緩衝（5 分鐘 / 300 點）。`servo.*` 為 Servo 追蹤狀態，`mag.heading` 由板上 QMC6310 磁力計提供，三者皆攝影站本地產生。
